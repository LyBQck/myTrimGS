...
class ParamGroup:
    def __init__(self, parser: ArgumentParser, name : str, fill_none = False, save_keys = []):
        ...
        for key, value in vars(self).items():
            ...
            if key.startswith("_"):
                ...
                key = key[1:]
            t = type(value)
            value = value if (not fill_none or key in save_keys) else None
            if shorthand:
                if t == bool:
...
class ModelParams(ParamGroup):
    def __init__(self, parser, sentinel=False, save_keys=[]):
        self.sh_degree = 3
        self._source_path = ""
        self._model_path = ""
        self._images = "images"
        ...
        self.eval = False
        super().__init__(parser, "Loading Parameters", sentinel, save_keys)
